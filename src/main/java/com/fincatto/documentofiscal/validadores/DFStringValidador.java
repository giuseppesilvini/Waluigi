...
public abstract class DFStringValidador {
    ...
    public static void placaDeVeiculo(final String placaVeiculo) {
        if (placaVeiculo != null) {
            final Matcher matcher = Pattern.compile("^([A-Z]{2,3}\\d{4}|[A-Z]{3,4}\\d{3}|[A-Z]{3}\\d[A-Z]\\d{2})$").matcher(placaVeiculo);
            if (!matcher.find()) {
    ...
    public static void inscricaoEstadual(final String inscricaoEstadual) {
        if (inscricaoEstadual != null) {
            final Matcher matcher = Pattern.compile("^(ISENTO|\\d{2,14}|)$").matcher(inscricaoEstadual);
            if (!matcher.find()) {
    ...
    public static void ncm(final String ncm) {
        if (ncm != null) {
            final Matcher matcher = Pattern.compile("^(\\d{2}|\\d{8})$").matcher(ncm);
            if (!matcher.find()) {
    ...
    public static void nve(final String nve) {
        if (nve != null) {
            final Matcher matcher = Pattern.compile("^[A-Z]{2}\\d{4}$").matcher(nve);
            if (!matcher.find()) {
